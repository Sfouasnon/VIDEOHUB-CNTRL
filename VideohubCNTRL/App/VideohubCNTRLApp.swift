...
struct VideohubCNTRLApp: App {
        ...
        if let sessionFlag = arguments.firstIndex(of: "--qa-session"),
           arguments.indices.contains(sessionFlag + 1) {
            ...
            qaDefaults = UserDefaults(
                suiteName: "com.videohubcntrl.VideohubCNTRL.QA.\(safeSession)"
            ) ?? .standard
            let customizationURL = FileManager.default.temporaryDirectory
                .appendingPathComponent(
                    "VideohubCNTRL-QA-\(safeSession)-Customizations.json",
                    isDirectory: false
                )
    ...
    var body: some Scene {
        Window("Videohub CNTRL", id: "main") {
            ContentView(store: store)
                .preferredColorScheme(.dark)
                .background {
                    WindowConfigurationView(
                        autosaveName: "VideohubCNTRL.MainWindowFrame",
